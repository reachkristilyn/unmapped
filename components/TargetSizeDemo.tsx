  ...
  return (
    <div className="mt-10 rounded-2xl bg-emerald-900 p-6">
      ...
      <p className="mt-2 text-emerald-100 text-sm">
        ...
        pointer, or a tongue-driven cursor.
      </p>
        ...
        {sizes.map(s => (
          <div key={s.label} className="text-center">
            <button
              ...
              aria-label={`${s.label} target, ${s.px} pixels`}
              className="rounded-full bg-emerald-300 hover:bg-emerald-200"
              style={{ width: s.px, height: s.px }}
            />
            <p className="mt-2 text-sm text-emerald-100">{s.label}<br />{s.px}px · {hits[s.label] || 0} hits</p>
          </div>
        ))}
      </div>
    </div>
